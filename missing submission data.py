...
def save_file(dict):
    ...
    file = os.path.join(os.path.join(os.getcwd(), 'missing submission data'), file_name+".csv")
...
for day in range(start_date,
    ...
    comments = api.search_submissions(limit=10, q='e', after=day, before=day+seconds_per_day)
    if len(comments) > 0:
